...
import { adcdLayerConfig, geoServerConfig } from '../config/layerConfig'
/**
 * 行政区划边界
 */
class AdcdLayer extends BaseVectorLayer {
  /**
   * 图层配置
  ...
  constructor(layerConfig) {
    let config = adcdLayerConfig
    if (layerConfig) {
      config = Object.assign(adcdLayerConfig, layerConfig)
    }
    super(config)
    this.minLevel = 4
    this.maxLevel = 1
  ...
  async load(map, adcd, datas) {
    ...
    if (adcd.length === 4) {
      geoServerParams.typeName = 'ZhejiangAdminDivisionRough:city'
    } else if (adcd.length === 6) {
      geoServerParams.typeName = 'ZhejiangAdminDivisionRough:county'
    }
    const filter = `admin_div_code=${mapAdcd}`
